...
app = Celery('celery_checker', broker='redis://:%s@localhost:9738/5' % os.getenv('REDIS_PASSWORD', None), include=['twoops.worker.celery_checker'])

@app.task
def check_tweet(entry):
    ...
    try:  # check if the tweet still on twitter
